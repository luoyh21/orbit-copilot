...
Windows 7 SP1 64 位必须下载名称带 `Win7-SP1-x64-portable.zip` 的绿色包，不能使用普通安装器，也不要再使用旧的 `Win7-SP1-x64-offline-setup.exe`。把 ZIP 完整解压到本机磁盘后直接运行 `Orbit-Copilot.exe`；包内固定携带 WebView2 109，不安装系统 WebView、不写系统目录、不要求管理员权限，目标内网机也不需要联网。详见 [Win7 绿色版](docs/Win7离线安装.md)和带截图的 [Win7 虚拟机验收报告](docs/Win7虚拟机验收报告.md)。Win7 与 WebView2 109 均已停止安全更新，只能作为隔离内网的临时兼容方案。
